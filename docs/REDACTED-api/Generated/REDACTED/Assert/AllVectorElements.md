---
layout: default
title: AllVectorElements
grand_parent: 
parent: Assert
has_children: False
permalink: /(REDACTED_FRAMEWORK)-api/(REDACTED).(REDACTED_OTHER_FRAMEWORK)/Assert/AllVectorElements
---
# Assert.AllVectorElements method (1 of 2)

Creates an action that asserts that all elements in a vector satisfy a condition

```csharp
public static AssertVectorElements<byte> AllVectorElements(GNServiceMethods propertyName, 
...
# Assert.AllVectorElements&lt;T&gt; method (2 of 2)

Creates an action that asserts that all elements in a vector satisfy a condition

```csharp
public static AssertVectorElements<T> AllVectorElements<T>(string propertyName, 
